...
import { BoxType, ImageType } from '../App';
...
function ImageDetail({ image, createMessage }: ImageDetailProps) {
  const [drawBoxes, setDrawBoxes] = useState<BoxType[]>([]);
  ...
  const onEyeIconClick = (box: BoxType) => {
  ...
  const onInputChange = (box: BoxType) => {
    updateBoxes(box);
  };

  const onCheckIconClick = (currentBox: BoxType) => {
    ...
    if (
      ...
      currentId === Math.max(...boxes.map(box => box.id))
    ) {
      setAddBoxMode(false);
    }
  ...
  const onImageClick = (newBox: BoxType) => {
    ...
    newBox.id = convertIdStrToInt(`${newBox.id}_${new Date().getTime()}`);
